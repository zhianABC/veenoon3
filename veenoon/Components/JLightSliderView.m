...
@implementation JLightSliderView
@synthesize delegate;
@synthesize topEdge;
...
- (id) initWithSliderBg:(UIImage*)sliderBg frame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self setIndicatorImage: [UIImage imageNamed:@"wireless_slide_light_n.png"]];
    }
...
- (void) resetScalValue:(float) scalValue {
    ...
    valueLabel.text = [NSString stringWithFormat:@"%0.0f", (scalValue)];
    curValue = scalValue;
    ...
    if (scalValue == minValue) {
        ...
        [self setIndicatorImage: [UIImage imageNamed:@"wireless_slide_light_s.png"]];
    }
