...
#import "AirCleanRightView.h"

@interface EngineerAirCleanViewCtrl () <CustomPickerViewDelegate>{
    ...
    BOOL isSettings;
    AirCleanRightView *_rightView;
    UIButton *okBtn;
}
@end

@implementation EngineerAirCleanViewCtrl
@synthesize _airCleanSysArray;
@synthesize _number;
- (void)viewDidLoad {
    ...
    if (_airCleanSysArray == nil) {
        _airCleanSysArray = [[NSMutableArray alloc] init];
    }
    ...
    [_selectSysBtn setTitle:@"空气净化" forState:UIControlStateNormal];
    ...
    for (int i = 0; i < self._number; i++) {
        NSMutableDictionary *dic = [self._airCleanSysArray objectAtIndex:i];
...
- (void) scenarioAction:(id)sender{
    ...
    NSMutableDictionary *dic = [self._airCleanSysArray objectAtIndex:index];
...
- (void) didConfirmPickerValue:(NSString*) pickerValue {
    ...
    NSString *title =  [@"空气净化" stringByAppendingString:pickerValue];
    [_selectSysBtn setTitle:title forState:UIControlStateNormal];
}
- (void) okAction:(id)sender{
    if (!isSettings) {
        _rightView = [[AirCleanRightView alloc]
