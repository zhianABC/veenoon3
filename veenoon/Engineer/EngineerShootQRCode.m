...
static EngineerShootQRCode* shootInstance = nil;


@interface EngineerShootQRCode ()<ZBarReaderDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
...
@implementation EngineerShootQRCode
...
+ (EngineerShootQRCode*)sharedShootCodeInstance{
    ...
    if(shootInstance == nil){
        shootInstance = [[EngineerShootQRCode alloc] init];
    }
...
- (void) initCameraShoot{
    ...
    int topSpace = StatusRect.size.height+44;
    
    
    UILabel *maskCover = [[UILabel alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-topSpace-60, SCREEN_WIDTH, 80)];
    ...
    cancelBtn.frame = CGRectMake(50, SCREEN_HEIGHT-50-topSpace, SCREEN_WIDTH-100, 40);
    ...
    photoAlbum.frame = CGRectMake(SCREEN_WIDTH-50, SCREEN_HEIGHT-55-topSpace, 44, 50);
...
- (void) imagePickerController: (UIImagePickerController*) reader didFinishPickingMediaWithInfo: (NSDictionary*) info {
    ...
    if(reader == _imagePicker)
    {
        ...
        IMP_BLOCK_SELF(EngineerShootQRCode);
    ...
    [_httpChecker requestWithSusessBlock:^(id lParam, id rParam) {
        ...
        SBJson4ValueBlock block = ^(id v, BOOL *stop) {
            ...
            if([v isKindOfClass:[NSDictionary class]])
            {
                ...
                if(code == 0)
                {
                    [block_self processScanResult:v];
                }
                else
                {
                    NSString *msg = [v objectForKey:@"msg"];
...
- (void) processScanResult:(NSDictionary*)result{
    ...
    if([result objectForKey:@"person"])
    {
              
    }
    else if([result objectForKey:@"user"])
    {
        
    }
    else if([result objectForKey:@"product"])
    {
           
    }
    else if([result objectForKey:@"event"])
    {
             
    }
    else if([result objectForKey:@"admission"])
    {
        NSDictionary *admission = [result objectForKey:@"admission"];
        if([admission objectForKey:@"user"])
        {
            
        }
        else if([admission objectForKey:@"person"])
        {
            
        }
    }
    else if([result objectForKey:@"company"])
    {
