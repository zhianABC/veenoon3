...
@property (strong, nonatomic) UIView *showYearView;
...
@synthesize showYearView;
/**
 默认滚动到当前时间
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle frame:(CGRect)frame {
...
/**
 滚动到指定的的日期
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle frame:(CGRect)frame scrollToDate:(NSDate *)scrollToDate{
    self = [super initWithFrame:frame];
    if (self) {
...
-(void)setupUI {
    ...
    self.showYearView = [[UIView alloc] initWithFrame:self.bounds];
